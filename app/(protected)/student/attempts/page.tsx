...
import { attemptMyListService } from "@/services/exam-service";
...
export default function StudentAttemptsListPage() {
  const [attempts, setAttempts] = useState<AttemptSummary[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    attemptMyListService().then(setAttempts).catch(() => {}).finally(() => setLoading(false));
  }, []);

  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-display font-medium text-espresso tracking-tight">My Attempts</h1>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {attempts.map((a) => (
            <Link
              key={a.attempt_id}
              href={`/student/attempts/${a.attempt_id}`}
