...
import type { practiceExamSchema } from "@/schemas/practice-exam-schema";
...
interface DetailsStepProps {
  file: File | null;
  form: UseFormReturn<PracticeExamFormValues>;
  creating: boolean;
  onSubmit: (data: PracticeExamFormValues) => void;
  onBack: () => void;
}
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <Field>
            <Label className="block text-xs font-medium text-bark mb-1">Practice test title *</Label>
            <input placeholder="e.g. Chapter 4 Review" className={inputCls} {...field} />
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button type="submit" disabled={creating} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-forest text-white text-sm font-medium hover:bg-forest-dark transition-colors disabled:opacity-50 cursor-pointer border-0">
          {creating ? "Creating..." : "Generate questions"}
          <ArrowRight size={16} />
        </button>
      </div>
    </form>
